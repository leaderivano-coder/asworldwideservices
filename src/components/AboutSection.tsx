...
import { MapPin, HeartHandshake, Eye, Maximize2, X } from 'lucide-react';
...
export const AboutSection: React.FC = () => {
  ...
  const teamMembers = [
    {
      ...
      subtitle: 'On-the-ground factory inspections & client liaison',
    },
    {
      id: 'team-2',
      ...
      subtitle: 'Direct university coordination & student support',
    },
    {
      id: 'team-3',
      ...
      subtitle: 'Supplier verification & export logistics',
    },
  ];
  ...
  const leadership = [
    {
      id: 'ceo',
      name: 'Mr. LOMBE ANDREW',
      title: 'Chief Executive Officer (CEO)',
      imageUrl: 'https://i.postimg.cc/0MP1Lcx8/lombe.jpg',
    },
    {
      id: 'coo',
      name: 'Miss KIRABO KIGGUNDU NANTONGO',
      title: 'Chief Operating Officer (COO)',
      imageUrl: 'https://i.postimg.cc/Lnr7cNBg/gift.jpg',
    },
  ];
  ...
  return (
    <section id="about" className="py-16 sm:py-24 bg-white">
          ...
          <div className="pt-10 mt-10 border-t border-[#E8E2D5]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {leadership.map((person) => (
                <div 
                  key={person.id}
                  className="bg-white rounded-2xl border border-[#E8E2D5] p-6 shadow-sm flex items-center gap-6 cursor-pointer group hover:border-[#C59B4B]/50 transition-colors"
                  onClick={() => setSelectedPhoto({
                    src: person.imageUrl,
                    title: person.name,
                ...
                >
                  <div className="relative">
                    <img
                      src={person.imageUrl}
                      alt={person.name}
                      className="w-24 h-24 rounded-full object-cover border border-[#E8E2D5] group-hover:border-[#C59B4B]/50 transition-colors"
                      referrerPolicy="no-referrer"
                    />
                    <div className="absolute bottom-0 right-0 w-7 h-7 rounded-full bg-[#0A2240] text-white border-2 border-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200">
                      <Maximize2 className="w-3 h-3" />
                    </div>
                  </div>
                  <div>
                    <h4 className="font-bold text-[#0A2240] text-lg">{person.name}</h4>
                    <p className="text-[#C59B4B] font-medium text-sm">{person.title}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Full Resolution Photo Modal — Works for ALL photos */}
      {selectedPhoto && (
        <div 
          ...
          >
            <div className="flex items-center justify-between pb-3 border-b border-[#E8E2D5] mb-3">
              <div>
                ...
                <p className="text-xs text-slate-500">{selectedPhoto.subtitle}</p>
              </div>
              <button
                type="button"
                onClick={() => setSelectedPhoto(null)}
